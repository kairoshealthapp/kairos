...
> [← Index](INDEX.md) · Long-form project context, identity, and persistent memory for Kairos sessions.
...
### MA rooming — Marisol Tysander (the attestation theater proof point)
...
- MAs click "Medications Reviewed" in seconds despite 50+ medication lists — Marisol Tysander review showed 6 attestation buttons in 9 minutes for a patient with 50+ meds
...
**Pre-probe structural diagnostic (2026-04-28).** `scripts/jwt-diagnostic.js` generates a Backend Services JWT using the same logic as `firekraker-monorepo/clinai/lib/epic-auth.js` (the source of truth for the kairos-auth Worker JWKS), decodes header/payload, and verifies every Epic-required field PASS/FAIL — plus fetches the public JWKS from `auth.firekraker.net` (confirmed Cloudflare-edge served via `cf-ray` + `server: cloudflare` headers) and cross-checks the JWT's `kid+alg` against the live key set. **First run 2026-04-28: all checks PASS, exit 0.** Empirically rules out our side as the cause of the `invalid_client` blocker — failure is definitively Epic-side app-config on apps 54107 and 54037. Run with `node scripts/jwt-diagnostic.js`; reads private key from `firekraker-monorepo/.env.master`. Does NOT call Epic's token endpoint.
