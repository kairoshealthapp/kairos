...
2. **Results Follow-Up** (filter = Ballard)
3. **Rx Request** (filter = Ballard)
4. **Patient Call** (filter = Ballard)
5. **Pt Advice Request** (filter = "Both of us")
6. **Custom search:** Results Follow-Up addressed to Brandon Sterne

Each box requires its own filter dialog because Epic's "Filter by Provider" UI shows the *ordering provider* (Ballard, Ballout, Morrison, Virk) — it cannot filter by the recipient nurse.
...
The nurse-provider pairing is a **configuration value**, not a runtime filter. Brandon → Ballard. Set once, applied automatically.
...
**Canonical example:** Sinks Pharmacy med list fax.
...
**Canonical example:** Phillips (DOE 4 stents 2025). HVC produced 20 chart-aware questions across 5 categories. Caught missing nitro, weight fluctuation, orthostatic BP, and discovered MRSA infection via chart-aware questioning.
...
- **Opening attribution:** "Mr. Ballard has reviewed your recent [lab/log/results]"
...
4. **What did Ballard say when Phillips was routed urgent?**
