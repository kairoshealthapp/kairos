...
1. **Pattern 13 — INSURANCE DENIAL CASCADE** observed end-to-end (Vanstone, 8-day persistent investigation across 4 Epic surfaces)
...
4. **Cross-note synthesis primitive** confirmed (Underwell: HVC absorbed two separate Ballard notes — primary plan + addendum — into one unified callback)
...
### CASE 25 — Vanstone (8-day insurance denial cascade)
...
**Patient:** 41F, Adler J Townsend FNP-BC primary, Ballard NP cardiology, Humana

**Full timeline (8 days, 3 Secure Chat threads, 2 Ballard exchanges, 2 patient communications):**

- **4/02:** NM SPECT ordered by Ballard
- **4/21 10:24 AM:** Secure Chat denial #1 from Genevieve Strathmore (PHS Mobile Cardiology Support Staff) — Homestate Health denied NM SPECT per Evolent Clinical Guideline 7312, requesting documentation of medical necessity for why exercise stress echo cannot be performed instead. Brandon forwarded to Ballard.
- **4/21 12:32 PM:** Ballard reply: "If the patient can exercise, can change to a stress echocardiogram."
...
- **4/21 1:19 PM:** Ballard reply: "Given the patient's inability to walk on a treadmill, would recommend Lexiscan Myoview stress test for further evaluation. Let whomever it may concern this is the reason why the Lexiscan Myoview stress test."
- **4/23 1:24 PM:** Vanstone, Carol N contacted Beatrix Kingsway (front desk).
...
- **4/24 7:53 AM:** Brandon Nurse Note — "Per Ballard, Lexiscan Myoview stress test scheduled 4/24 has been cancelled. Pre-cert appeal also cancelled per provider. Coronary CT angiogram ordered given patient's symptoms of chest pain, SOB, nausea, and diaphoresis with known CAD s/p PCI with stenting. Awaiting insurance approval." Modules accepted: Orders.
- **4/29 2:32 PM:** Secure Chat denial #2 from Marielle Tannenbaum — CTA Coronary Arteries denied. "The reason for this is that the following notes have not been given: a doctor's note with a reason why a heart test where you walk with heart pictures (Stress Echocardiogram) cannot be done. A peer to peer is available by calling 800-308-2615 but would need to be done today. You would need the patient name, dob, Member ID# 16337356 and Tracking # 098121753502. We can also resubmit the request with any new results, documentation or note addendums. Thank you. Leona Petrov (8789)"
- **4/29 2:33 PM:** Brandon added Roland P Ballard, NP to the Secure Chat thread.
- **4/29 2:34 PM:** Brandon: "Mr Ballard, FYI" + thumbs-up reaction.
...
- **4/29 3:36 PM:** MyChart message sent to Ms Vanstone. Status: "This Phelps Health MyChart message has not been read." Encounter still open ("This encounter is not signed. The conversation may still be ongoing.")
...
- **Cross-channel investigation consolidation** — Secure Chat (×2 staff: Genevieve Strathmore, Marielle Tannenbaum) + In Basket (×2: front desk Kingsway, then Brandon) + encounter notes + orders + MyChart all collapse to one persistent object.
...
### CASE 26 — Vanstone closure outbound (Pattern 13 second half)
...
> Ms Vanstone,
> 
> Unfortunately, your insurance has denied the heart CT scan that Mr. Ballard ordered. This is the second test they have denied, and we understand how frustrating this is.
> 
> Mr. Ballard's recommended next step is a heart catheterization (heart cath). This is a procedure where a small tube is guided through a blood vessel to take detailed pictures of the arteries around your heart. Given your history of coronary artery disease with prior stent placement and the symptoms you have been experiencing, this is the best path forward to find answers. We are hopeful insurance will approve this procedure.
> 
...
> 
> Please reply to this message at your earliest convenience so we can update Mr. Ballard and determine next steps.
> 
...
**Source:** Ballard Result Note 4/29 with TTE Complete read by Donovan Morrison MD on 4/29:
1. Echocardiogram
...
> 
> We did see two minor valve findings. There is a trivial, meaning very slight, amount of leakage through your aortic valve, and a mild amount of leakage through your mitral valve. Mr. Ballard has reviewed these findings and is not making any changes to your current treatment based on this echo.
> 
...
- **Provider-authority preserved** — "Mr. Ballard has reviewed these findings and is not making any changes" rather than "the report says no changes."
...
- **3:25 PM** — Ballard signed Note #1 (primary plan):
...
- **3:27 PM** — Ballard signed Note #2 (addendum providing supporting context):
  1. BNP on 4/1/2026 was normal at 200
  2. Holter monitor pending related to atrial fibrillation with increase in carvedilol
- **~3:50 PM** — Brandon called patient to relay plan. HVC absorbed both Ballard notes (primary + addendum) into one unified callback Nurse Note (Opus 4.7).
- **4:06 PM** — Encounter signed.

**HVC callback Nurse Note (Opus 4.7):**

> Spoke with patient by phone to relay plan per Ballard.
> 
...
1. **Cross-note synthesis** — Ballard's plan came in two separate Epic notes (3:25 primary + 3:27 addendum). HVC merged them cleanly into 6 numbered items. Items 3 (BNP) and 4 (Holter) come from the addendum; items 1/2/5/6 come from the primary plan. The synthesis is invisible to the reader — looks like one coherent plan.
2. **Concept reordering for patient comprehension** — Ballard's primary order was "discontinue / continue / BP log / swelling / consideration." HVC reordered the patient-facing version to: action items first (1-2), reassurance context (3-4), follow-up obligations (5-6). Same content, optimized sequence for spoken delivery.
3. **Implicit handoff between notes** — "Spoke with patient by phone to relay plan per Ballard" attribution preserves provider authority. Same pattern as the 2:17 PM Nurse Note ("Forwarded to Ballard for review"). Cross-output consistency holds across all three documents in this encounter.
...
13. **NEW: Test-cascade timeline view** (Vanstone — multi-order, multi-denial chain in one persistent object)
...
18. **NEW: Auto-framing by auth state** (Vanstone — denial-cascade context auto-detected, prevents Type 4 edits)
...
25. **NEW: Authorization state monitoring** (Vanstone — proactive surveillance of pending pre-certs, denials, peer-to-peer deadlines)
...
28. **Multi-channel outreach correlation** (Vanstone — voicemail + MyChart auto-correlate in outbound message)
29. **Read-receipt as state signal** (Vanstone — `Communication.received` drives card escalation timing)

### Inbound Source Channels (NOW 7 — added authorization-deadline source)

- 5 In Basket boxes (morning)
- Secure Chat threads (afternoon)
- **NEW: Authorization deadline events** (Vanstone — peer-to-peer deadline as proactive surveillance trigger)
...
| 4 | SYNTHESIS + DOSE CHANGE + LAB CLUSTER | Calderwood |
| 5 | SYNTHESIS + CHOICE | Brexley |
| 6 | SYNTHESIS + HANDOFF | Drennan |
| 7 | URGENT | Esselbach |
| 7b | ASYNC PRE-CALL STRUCTURED INQUIRY | Phillips, Stockbridge, Underwell (full lifecycle) |
| 8 | CONTRADICTION | Maundrell |
| 9 | TRANSACTIONAL REPLY | Norreys |
| 10 | COORDINATION | Lyttleton, Trumble, Halbrook |
| 11 | ADMINISTRATIVE (Layer 1) | Sinks |
| 12 | SCOPE-CONSTRAINED PATIENT QUESTION | Quennell-redux |
| **13** | **INSURANCE DENIAL CASCADE** | **Vanstone** |
...
**Late shift (4 cards):**
- Vanstone (denial cascade init) — Type 4 (input augmentation needed for denial framing — eliminable with auth-state primitive)
- Vanstone (closure outbound) — clean
...
- 19 cards observed
- 14 clean outputs
- 4 eliminable-rail edits (Brexley, Lyttleton, Quennell-redux v1, Vanstone init)
...
- [x] Pattern 13 (INSURANCE DENIAL CASCADE) observed end-to-end through Vanstone 8-day investigation
...
- [x] Auto-framing by auth state primitive named (Vanstone)
- [x] Multi-channel outreach correlation primitive named (Vanstone)
- [x] Read-receipt as state signal primitive named (Vanstone)
- [x] Voicemail variant + phone callback state machine primitives named (Wexbury)
- [x] Test-cascade timeline view primitive named (Vanstone)
- [x] Authorization state machine primitive named (Vanstone)
- [x] Authorization state monitoring as Layer 1 surface (Vanstone)
