...
**Naming reminder:** patient/clinician names below use the scrubbed fakes from the 4/29 shift files (e.g., Aldington = morning's Boultes, Lyttleton = Patton, Vanstone = Brown, Wexbury = Ramey). The mapping table is local-only at `docs/.name-scrub-mapping-2026-04-29.md`.
...
│ Loxley's Result Note      │ HVC-drafted, pinned to source.    │
...
| 4 | SYNTHESIS + DOSE CHANGE + LAB CLUSTER | Med change + multi-lab follow-up (Calderwood Crestor 40 + lipid + hepatic) | All 4 (order pad shows lab cluster) | Generate Note + MyChart + dose diff + lab cluster | v1 |
...
| 6 | SYNTHESIS + HANDOFF | Cross-specialty referral, no orders fire from cardiology side (Drennan PCP handoff) | Source / Nurse Note / MyChart / (Order pad shows handoff routing) | Generate Note + MyChart + Route to PCP | v1 |
...
| 10 | COORDINATION | Multi-task across boxes, persistent investigation, multi-party scheduling (Lyttleton, Trumble Secure Chat, Halbrook DME PA) | Source / Nurse Note / MyChart-or-Phone / (Order pad usually empty) | Generate Reply + Forward to Provider with options | v1 surface; Pattern 10 reroute v3-v4 |
...
Source: [KAIROS-SESSION-2026-04-29.md](KAIROS-SESSION-2026-04-29.md) Section 4 + CASE 1 (Aldington) and CASE 3 (Calderwood). Field-level derivation rules below; each field has a **derivation source**, a **fallback**, and a **confidence flag** that surfaces in UI when the derivation is weak.
...
Calderwood Crestor 40mg → DC 20mg pattern is the canonical example ([KAIROS-SESSION-2026-04-29.md](KAIROS-SESSION-2026-04-29.md) CASE 3).
...
Calderwood Lipid Panel + Hepatic Function Panel both Future 7/29/2026 is the canonical example.
...
The card UI surfaces what Epic does not: a single object that holds an open clinical question across days, channels, encounters, and orders. Source: [KAIROS-SESSION-2026-04-29-EVENING.md](KAIROS-SESSION-2026-04-29-EVENING.md) CASE 25 (Vanstone, 8 days, 4 Epic surfaces) + CASE 28 (Underwell Pattern 7b lifecycle).
...
investigation = {
  ...
  clinicalQuestion:   string     (canonical short phrase: "amlodipine BP/edema",
                                  ...
                                  "scheduling escalation Cuculich-equivalent")
...
When a Pattern 13 outreach fires both MyChart and phone simultaneously (Vanstone closure), the outbound MyChart message includes a voicemail-acknowledgment line:
