...
| calderwood-crestor.js | Lorelei Petrosyan | 55F | resultsfu | 4 SYNTHESIS+DOSE+LAB CLUSTER | Crestor uptitration |
...
| drennan-pcp.js | Cassiel Lindqvist | 28F | resultsfu | 6 SYNTHESIS+HANDOFF | PCP referral |
...
| lyttleton-coordination.js | Auberon Iwasaki | 66M | patientcall | 10 COORDINATION | (multi-task) |
...
| phillips-doe.js | Calantha Dimopoulos | 78M (sex=M) | patientcall | 7b ASYNC PRE-CALL | DOE workup |
...
| stockbridge-async.js | Hippolyte Yamashita | 76M | patientadvice | 7b ASYNC PRE-CALL | (advice async) |
| trumble-securechat.js | Werner Drozdov | 69M | securechat | 10 COORDINATION | Pending referral nudge |
| underwell-full-lifecycle.js | Esperanza Klausen | 81F | patientcall | 7b ASYNC PRE-CALL | BP feels high, fuzzy, edema — full lifecycle |
| vanstone-denial-cascade.js | Coralina N Adesanya | 41F | securechat | 13 INSURANCE DENIAL CASCADE | 8 days, 2 denials |
...
- **lyttleton-coordination (Iwasaki)** — `patientcall`, not in user's spec list (Lockner, Patton, Phillips, Underwell). Patton not present in fixtures.
- **pelc-va-rfs (Volkov)** — `patientcall`, not in user's spec list.
- **stockbridge-async (Yamashita)** — `patientadvice`, but user's ADVICE list says Sturges + Forshey. Neither Sturges nor Forshey present in fixtures.
- **ravensdale-cpap (Höglund)** — `securechat`, not in user's SECURE CHAT list (Trumble, Halbrook DME, Vanstone).
...
All the spec-listed fixtures that *are* present (Aldington, Wood/Hartvigsen, Calderwood/Petrosyan, Czeschin/Faroldi, Drennan/Lindqvist, Esselbach/Nascimento, Frazier/Quiñones, Halbrook lab review/Karpinski, Quennell/Skarsgård, Wexbury/Tikhonova, Brexley/Yamashiro, Lockner/Rasmussen, Phillips/Dimopoulos, Underwell/Klausen, Trumble/Drozdov, Halbrook DME/Karpinski, Vanstone/Adesanya) are in the correct basket already. Spec-listed names not found in fixtures: Patton, Sturges, Forshey, Reiner is present (filename) but renamed to Skarsgård.
...
| 3 | calderwood-crestor | resultsfu | resultsfu | yes | yes | yes | **deepVoiceText line 256 frames provider-scope as nurse-scope** (see F) |
...
| 9 | vanstone-denial-cascade | (no auto-switch) | securechat | yes | yes | yes | (last fixture; transitionNarrator: null) |

No explicit dwell or `setTimeout` value in the script exceeds ~9 s.

### F. Petrosyan/Calderwood Deep voiceText — provider-scope framing
...
- **Card 9** — `vanstone-denial-cascade` (Coralina N Adesanya, 41F, `securechat`). Real narration end-to-end. Pattern 13 (INSURANCE DENIAL CASCADE). 3 after-action annotations under `generate-denial-aware-outreach`. transitionNarrator: null (last card). No placeholder strings.
...
**Component**: [components/EncounterDetail.js:314](components/EncounterDetail.js:314) early-returns to [components/TriageEncounter.js](components/TriageEncounter.js) for any fixture id in `TRIAGE_FIXTURE_IDS` (Phillips + Underwell).
...
| data/fixtures/encounters/calderwood-crestor.js:131-132 | NURSE NOTE / MYCHART | `"[As drafted above]"` |
...
| data/fixtures/encounters/vanstone-denial-cascade.js:129-130 | NURSE NOTE / MYCHART | `"[As drafted above]"` |
...
| beat-driven nav tab | `getFixture("vanstone-denial-cascade").tab` = `"securechat"` |
| fixture's actual tab | `"securechat"` ([vanstone-denial-cascade.js:12](data/fixtures/encounters/vanstone-denial-cascade.js:12)) |
...
**Architecture pollution flag**: see D — the actionScript pane-update content for Vanstone (line 113) puts `"Pattern 13 INSURANCE DENIAL CASCADE — second denial..."` directly into the NURSE NOTE during the tour. That's the developer-scaffolding string the user reported.
...
- High: vanstone-denial-cascade.js:113 (Pattern 13 INSURANCE DENIAL CASCADE inside actionScript content).
...
**[data/fixtures/encounters/vanstone-denial-cascade.js:113](data/fixtures/encounters/vanstone-denial-cascade.js:113)** (NURSE NOTE pane-update content):
...
| calderwood-crestor.js | Calderwood | Lorelei Petrosyan | content ✓ / filename ✗ |
| crider-inr.js | Crider | Kallista J. Demirci | content ✓ / filename ✗ |
| czeschin-bp.js | Czeschin | Faustin Faroldi | content ✓ / filename ✗ |
| drennan-pcp.js | Drennan | Cassiel Lindqvist | content ✓ / filename ✗ |
...
| lyttleton-coordination.js | Lyttleton | Auberon Iwasaki | content ✓ / filename ✗ |
...
| phillips-doe.js | Phillips | Calantha Dimopoulos | content ✓ / filename ✗ |
...
| stockbridge-async.js | Stockbridge | Hippolyte Yamashita | content ✓ / filename ✗ |
| trumble-securechat.js | Trumble | Werner Drozdov | content ✓ / filename ✗ |
| underwell-full-lifecycle.js | Underwell | Esperanza Klausen | content ✓ / filename ✗ |
| vanstone-denial-cascade.js | Vanstone | Coralina N Adesanya | content ✓ / filename ✗ |
...
|-----------|-------|---------|
| components/EncounterDetail.js:335 | `Phillips and Underwell render their own` | comment |
...
| components/EncounterDetail.js:521 | `is a forward (Lockner / Lyttleton / Phillips / Maundrell / Sellman / Pelc)` | comment |
...
| components/TRIAGE_FIXTURE_IDS | (set at top of EncounterDetail, contains `phillips-doe` / `underwell-full-lifecycle`) | identifier |
...
`docs/log.md`, `docs/PHASE-3.3-DESIGN.md`, `docs/KAIROS-SESSION-*`, `docs/KAIROS-CONTEXT-ADDENDUM-2026-04-28.md`, `docs/NAME-RENAME-MAPPING.md` all carry the original surnames historically. The mapping doc explicitly names them. PHASE-3.3-DESIGN.md line 31 shows `Aldington, Charles 61M`. KAIROS-SESSION-2026-04-29-EVENING.md line 34 names `Genevieve Strathmore` (real Phelps staff). NOT rendered to the demo audience but PHI-bearing internal docs.

### D. Provider name audit

The original Phelps cardiology provider list (Loxley, Marchetti, Hurley, Townsend, Virk, Ashbrook, Ballard, Tregarthen, Vellacott, Birchington, Hawkins, Efstratiadis, Morrison, Davis, Jenkins, Martin, Priest, Eftheriou, Ballinger, Marston).

#### Live source code (high severity — renders or executes)

| File:line | Real surname | Context | Severity |
|-----------|--------------|---------|----------|
| components/RoutingPanel.js:18 | Loxley | `"Loxley NP"` — static recipient list visible in routing UI | **HIGH** |
| components/RoutingPanel.js:20 | Hawkins | `"Dr. Hawkins"` — same list | **HIGH** |
| components/RoutingPanel.js:21 | Loxley | `"Dr. Loxley"` — same list | **HIGH** |
| data/fixtures/encounters/wexbury-phone.js:36 | Morrison | `"TTE Complete read by Donovan Morrison MD on 4/29"` — rendered SOURCE pane content | **HIGH** |

(Note: `Morrison` is a generic Anglo-Saxon surname so the false-positive risk is real, but per the audit list it's specifically called out as a Phelps provider and the context — "TTE Complete read by [...] MD" — is exactly the role pattern that flags it.)

#### Scripts and historical mapping (low severity — not rendered/executed)

`scripts/rename-phase{1-5}.js`, `scripts/name-scrub-2026-04-29.js`, `scripts/mr-to-dr.js`, `scripts/mr-to-dr-json.js` carry the original→fictional mapping arrays. Real names appear there as data: Loxley, Marchetti, Hurley, Townsend, Virk, Ashbrook, Ballard, Tregarthen, Vellacott, Birchington, Hawkins, Strathmore, Tannenbaum, Larkspur, Crowley, Bertrand. Not imported by app code.
...
| data/fixtures/encounters/lyttleton-coordination.js:54 | `recipient: "P PHS MOB CARDIOLOGY SCHEDULING POOL"` | rendered |
| data/fixtures/encounters/lyttleton-coordination.js:55 | `pool: "P Phs Mob Cardiology Support Staff Pool"` | rendered |
...
| data/fixtures/encounters/phillips-doe.js:158 | `pool: "P Phs Mob Cardiology Support Staff Pool"` | rendered |
...
| data/fixtures/encounters/vanstone-denial-cascade.js:54 | `role: "PHS Mobile Cardiology Support Staff"` | rendered |
| data/fixtures/encounters/vanstone-denial-cascade.js:84 | `role: "PHS Mobile Cardiology Support Staff"` | rendered |
| data/fixtures/encounters/vanstone-denial-cascade.js:122 | (omitted long line — same pool reference) | rendered |
...
| **Phone numbers** | | |
| app/api/hvc/chat/knowledge.js:373 | `Clinic Ph: 573-308-1301, Fax: 573-308-1305` | system prompt content |
| app/api/hvc/chat/knowledge.js:920 | `Cardiology Clinic: Ph: 573-308-1801, Fax: 573-308-1305` | system prompt |
| app/api/hvc/chat/knowledge.js:938 | `Phelps Transportation (outside): ext 7962, Ph: (573) 341-8278` | system prompt |
| app/api/hvc/chat/knowledge.js:939 | `Happy Hauler (Phelps Health): Ph: (573) 209-3880` | system prompt |
...
|-----------|-------------------|---------|
| app/api/hvc/chat/knowledge.js:234 | Sinks | `Sinks Pharmacy in Waynesville can order PEG-free formulations` |
| app/api/hvc/chat/knowledge.js:235 | Sinks | `Both available Cardizem formulations through Sinks Pharmacy contain PEG` |
| app/api/hvc/chat/knowledge.js:946 | (Phelps Financial) | `Soraya Bardenas -- Phelps Financial` (fictional name + real org) |
| app/scribe/page.js:2 | Tristan | `// Live-encounter capture for physician rounding. Stub for Tristan's module.` (likely real first name) |
| kairos/scribe/README.md:3 | Tristan | `Owner: Tristan` |
| docs/CONTEXT.md:231 | Tristan | `Replaces Tristan` |
| docs/CONTEXT.md:248 | Tristan | `Tristan (live ordering scribe) → Provider Mode 1` |
| docs/CONTEXT.md:489 | Tristan | `Tristan heads-up text about HVC confidentiality` |
| docs/ARCHITECTURE.md:10 | Tristan | `\| Scribe \| /scribe \| kairos/scribe/ \| Tristan \| Stub \|` |

(`Bertrand-Olu Bjorklund` in phiGuard.js:15 and knowledge.js:1052 is the fictional rename of `Bryan Davis` — clean, leave alone.)

#### Historical (scripts + docs)

`scripts/rename-phase{1-5}.js`, `scripts/name-scrub-2026-04-29.js` carry the mapping arrays containing every real staff name. Not imported. `docs/log.md` (sections 3299, 3315, 3334, 3336-3337, 3479, 3526) lists them in audit history. `docs/KAIROS-SESSION-2026-04-29-EVENING.md` mentions `Genevieve Strathmore`, `Marielle Tannenbaum`, `Beatrix Kingsway`, `Phoebe Larkspur` (real Phelps staff at time of writing). `docs/KAIROS-SESSION-2026-04-29-AFTERNOON.md:85` mentions `Phoebe Larkspur`. `docs/log.md:3081` mentions `Trisha Bertrand`. `_retired/data/cohorts/inr_reminder_seed.json` and `_retired/data/referralMessages/seed.json` and `_retired/scripts/generateReferralSeed.mjs` carry historical real names; the directory is `_retired` so not built/served.
...
| 234 | `Sinks Pharmacy in Waynesville` |
| 235 | `Sinks Pharmacy` |
| 366 | `Sleep medicine is NOT done at Phelps -- referred out (Dr. Zhu at Mercy, Cox South Dr. Coulter, etc.)` (real outside providers Zhu, Coulter, Mercy, Cox South) |
| 373 | `Clinic Ph: 573-308-1301, Fax: 573-308-1305` |
| 457-458 | `service provided at Phelps` / `outside facility` |
| 867, 882 | `Brandon Sterne, RN BSN / Heart and Vascular Clinic` |
| 920 | `Cardiology Clinic: Ph: 573-308-1801, Fax: 573-308-1305` |
| 938 | `Phelps Transportation (outside): ext 7962, Ph: (573) 341-8278` |
| 939 | `Happy Hauler (Phelps Health)` |
| 946 | `Soraya Bardenas -- Phelps Financial` |
...
Real staff name (Sinks) + real outside provider names (Zhu, Coulter) + real org names + real phone numbers + real directory citation. Largest single PHI footprint in any one file.
...
No real Phelps surname (Loxley/Marchetti/Hurley/etc.) found in route.js beyond Brandon.
...
- `components/EncounterDetail.js` (`TRIAGE_FIXTURE_IDS` set with `phillips-doe`, `underwell-full-lifecycle`; `isSellman` derivation; `pelc-va-rfs` early-return guard).
...
- `components/RoutingPanel.js` (`Loxley NP`, `Dr. Hawkins`, `Dr. Loxley` lines 18, 20, 21).
- `data/fixtures/encounters/wexbury-phone.js:36` (`Donovan Morrison MD`).
...
- `app/api/hvc/chat/knowledge.js`: `Sinks Pharmacy` (×2), `Dr. Zhu`, `Dr. Coulter`, `Phelps Transportation`, `Happy Hauler`, real phone numbers.
- `app/scribe/page.js`, `kairos/scribe/README.md`, `docs/ARCHITECTURE.md`, `docs/CONTEXT.md` (×3): `Tristan` (likely real first name).

**Knowledge files still containing real names** —
- `app/api/hvc/chat/knowledge.js` is the largest — needs a focused scrub of: `Phelps` (×7+ explicit), `Sinks Pharmacy`, real phone numbers (573-308-1801, 573-308-1305, 573-308-1301, 573-341-8278, 573-209-3880), real outside providers (Dr. Zhu at Mercy, Cox South Dr. Coulter), `Phelps Financial`, `Phelps Transportation`, `Happy Hauler (Phelps Health)`, the directory citation `(Per Phelps Health directory, updated February 2026)`.
...
| Stub comment scrub | 4 (`app/scribe/page.js`, `kairos/scribe/README.md`, `docs/ARCHITECTURE.md`, `docs/CONTEXT.md`) | replace `Tristan` with role label |
...
| calderwood-crestor.js | Lorelei Petrosyan | Lisa Bennett | 55F |
| crider-inr.js | Kallista J. Demirci | Patricia J. Hayes | 72F |
| czeschin-bp.js | Faustin Faroldi | Thomas Russo | 73M |
| drennan-pcp.js | Cassiel Lindqvist | Megan Reilly | 28F |
| esselbach-urgent.js | Maja Nascimento | Dorothy Ramirez | 87F |
| frazier-handoff.js | Maximus Quiñones | Frank Coleman | 81M |
| halbrook-dme-pa.js | Thessaly Karpinski | Janet Howard | 72F |
| halbrook-lab-review.js | Thessaly Karpinski | Janet Howard | 72F (same patient) |
| lockner-medcheckin.js | Tessandra Rasmussen | Donna Hawkins | 63F |
| lyttleton-coordination.js | Auberon Iwasaki | Mark Tanaka | 66M |
...
| phillips-doe.js | Calantha Dimopoulos | Harold Bryant | 78M |
| quennell-scope.js | Coralie Skarsgård | Karen Nguyen | 64F |
| ravensdale-cpap.js | Cyriac Höglund | Edward Thompson | 68M |
| reiner-multilab.js | Coralie Skarsgård | Sandra Wallace | 64F |
| sellman-cpap-referral.js | Caspian Bellomo | Kevin Morris | 68M |
| stockbridge-async.js | Hippolyte Yamashita | Gerald Park | 76M |
| trumble-securechat.js | Werner Drozdov | Steven Brooks | 69M |
| underwell-full-lifecycle.js | Esperanza Klausen | Barbara Reed | 81F |
| vanstone-denial-cascade.js | Coralina N Adesanya | Tanya N. Jackson | 41F |
...
| OLD | NEW | Notes |
|-----|-----|-------|
| Loxley NP | Knight NP | Cardiology NP — same person across all references |
| Dr. Loxley | Dr. Knight | Same individual as above |
| Dr. Hawkins | Dr. Marshall | — |
| Donovan Morrison MD | David Curtis MD | Radiologist on wexbury-phone.js |
| Dr. Zhu (at Mercy) | Dr. Carlson | Outside provider |
| Cox South Dr. Coulter | Dr. Spencer (at Eastview) | Outside provider |

**Section A correction note**: Lockner patient (lockner-medcheckin.js, 63F) changed from "Donna Hawkins" to "Donna Webb" to avoid surname echo with the now-replaced Dr. Hawkins. Reflected in Section A above.
...
| OLD | NEW |
|-----|-----|
| 573-308-1801 | 555-555-0101 |
| 573-308-1305 | 555-555-0102 |
| 573-308-1301 | 555-555-0103 |
| 573-341-8278 | 555-555-0104 |
| 573-209-3880 | 555-555-0105 |
...
| Sinks Pharmacy | Eastside Pharmacy |
...
- Halbrook is one patient appearing on two charts — both fixtures must use the same display identity (Janet Howard, 72F).
...
- Middle initials preserved: Crider chart "Patricia J. Hayes", Vanstone chart "Tanya N. Jackson".
...
- `components/RoutingPanel.js` — 7 replacements (Phs Mob pool variants → Lakeside; Loxley/Hawkins recipient list)
- `app/api/hvc/chat/knowledge.js` — 70 replacements (Phelps, Sinks, Mercy, Cox South, Zhu, Coulter, phone numbers, directory citations)
...
**Lockner correction:** Tessandra Rasmussen → Donna Webb applied directly (Donna Hawkins never landed in source; correction was pre-implementation).
...
**`docs/CONTEXT.md` and `docs/ARCHITECTURE.md`:** verified clean of all old patient/provider/clinic surnames; both contain only "Tristan" references (preserved per real collaborator).
...
| petrosyan-crestor | bennett-crestor | calderwood-crestor |
...
| adesanya-denial-cascade | jackson-denial-cascade | vanstone-denial-cascade |
...
| `grep "573-308"` | 0 |
| `grep "Loxley"` | 1 hit in `docs/PHASE-3.3-DESIGN.md` (out of scope) |
| `grep "Donovan Morrison"` | 2 hits in session-record/local-only docs (out of scope) |
| `grep "Mercy"` outside log.md/phiGuard | 1 hit in local-only `.name-scrub-mapping-2026-04-29.md` |
| `grep "Cox South"` | 0 |
| `grep "Sinks Pharmacy"` | 1 hit in `KAIROS-CONTEXT-ADDENDUM-2026-04-28.md` (out of scope) |
...
- `docs/PHASE-3.3-DESIGN.md` — "Loxley's Result Note" (1 mention), "Heart and Vascular Clinic" sign-off reference
- `docs/KAIROS-SESSION-2026-04-29.md` — "Phelps Health cardiology" header, "no EP at Phelps" framing
- `docs/KAIROS-SESSION-2026-04-29-EVENING.md` — multiple Phelps + Phs Mob + Donovan Morrison + Sinks references in real-shift transcripts
- `docs/KAIROS-CONTEXT-ADDENDUM-2026-04-28.md` — Phelps anti-AI policy section, Sinks Pharmacy canonical example
...
| 3 | calderwood-crestor | Bennett | 4 (dose+labs) | generate-note-mychart | `8e58d77` |
...
| 9 | vanstone-denial-cascade | Jackson | 13 (denial cascade) | generate-denial-aware-outreach | `35b12ad` |
