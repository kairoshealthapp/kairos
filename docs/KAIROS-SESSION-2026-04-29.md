...
### Surface 1 — Results Follow-Up (5 of 20 patients filtered Provider=Hardenkvist)

- Aldington (61M, CTA chest, mild AS) — SYNTHESIS + NEW ORDER
- Brexley (63F, lipid panel, statin choice) — SYNTHESIS + CHOICE
- Hesperdale (55F, lipid + chem-8, Crestor 40 increase) — SYNTHESIS + DOSE CHANGE + LAB CLUSTER
- Wendelfaer (28F, CTA abdomen + incidental pancreatic cyst → PCP handoff) — SYNTHESIS + HANDOFF
- Esselbach (87F, BNP 1024 + pre-op surgery 5/14) — URGENT (phone)

### Surface 2 — Rx Request (6 of 6 patients filtered Provider=Hardenkvist)

Brandon's refill rule: verify last appt ≤12mo + future appt scheduled + dx match → fill 365 days.

- Felgenhauer, Gallichan, Brindelhart, Inkpen, Jorgan — auto-eligible Layer 1
- Veldenmoor (Jardiance, Antihyperglycemic Protocol FAILED) — HOLD, requires nurse review

5/6 auto-clear pre-arrival in Kairos Layer 1.

### Surface 3 — Patient Call (4 of 28)

- Kvalheim (66M) — single patient with 2 sub-tasks routed by front desk Adelaide Westkander:
...
- Maundrell (74M) — INR overdue template, patient replied: *"Dr. M told me to be taking only the other two medications and also said I don't have to take the blood test anymore."* CLINICAL CONTRADICTION: chart shows active warfarin. Cannot resolve from chart alone — must forward to Skarsdale. HOLD all action.
...
- ~4 require real cognitive engagement (Veldenmoor, Esselbach, Brexley, Maundrell)
...
4. **SYNTHESIS + DOSE CHANGE + LAB CLUSTER** (Hesperdale) — most field-heavy variant
5. **SYNTHESIS + CHOICE** (Brexley) — two-stage with conditional staged orders
6. **SYNTHESIS + HANDOFF** (Wendelfaer) — cross-specialty referral
...
10. **COORDINATION** (Kvalheim) — multi-task across boxes, persistent investigation
...
│ Hardenkvist's Result     │ HVC-drafted, pinned to source.         │
...
│ Body HVC-drafted     │   Cosign: Hardenkvist                      │
├──────────────────────┴───────────────────────────────────────┤
...
SYNTHESIS + NEW ORDER. HVC dual output clean. TTE Complete ECH110 STERNE BR ordered, Future / 4/29/2026 First Available / Routine, Mild AS + Mild AR diagnoses, Hardenkvist cosign. Done.
...
### CASE 3 — Hesperdale (~14 actions, 4-5 min)
...
### CASE 4 — Wendelfaer (Input Augmentation Pattern Discovered)
SYNTHESIS + HANDOFF. Hardenkvist's note had typo "**Lease** forward to PCP" (P missing). Brandon disambiguated by giving HVC explicit context: "i have forwarded the results to PCP as requested." HVC then rendered both registers correctly.
...
## Section 6 — Kvalheim Case (Patient Call) + Layer 1 Reroute Agent

Kvalheim (66M) called wanting earlier appointment than Tregarthen's September availability.
...
### Roadmap Path
- **v1:** Surface alternates ranked by availability on COORDINATION card. Default = "Forward to Hardenkvist with options."
...
BARNES JEWISH / WASH U (ST. LOUIS): Tregarthen, Vellacott, Birchington, Reardon Eldenfaer (ablation)
ST. LUKE'S / CLEVELAND CLINIC (ST. LOUIS): Jasper Vesperwild, Norhaven, Conrad Pelfridge
MERCY: Lakshmi Quintannon (St. Louis), Pranav Rendelman (Springfield), Wei-Lin Tarkenbridge (Springfield)
...
COX HEALTH (SPRINGFIELD): Marcus Selwyn Brindleforth
```
...
- Sub-specialty annotations (Reardon Eldenfaer = ablation/"hit table")
...
1. **Refill auto-processing** — Veldenmoor-excluded, 5/6 of Rx Request box clears autonomously
2. **Referral rerouting** — Kvalheim case, with HVC referral directory as foundation
...
- Hesperdale ~14 discrete actions, ~4-5 min
...
- 7:00 AM walk in
- 7:50 AM 4/5 Results Follow-Up cleared (Aldington, Brexley, Hesperdale, Wendelfaer)
...
Workflow: create Anticoagulation-Warfarin Visit encounter, document, plan, route Skarsdale cosign. Two paper mdINR faxes on desk: patient name, DOB, INR, draw date, **full historical trend chart printed on form**.
...
- [x] 5 patient round-trips observed (Aldington, Brexley, Hesperdale, Wendelfaer, Esselbach)
