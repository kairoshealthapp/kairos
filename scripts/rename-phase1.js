// Phase 1 systematic rename across data/fixtures/encounters/*.js
// Order matters: longer/more-specific strings first.

const fs = require("fs");
const path = require("path");

const dir = path.resolve(__dirname, "..", "data", "fixtures", "encounters");
...
const pairs = [
  ...
  // ADMIN / SUPPORT (multi-word, before single-name fallbacks)
  ...
  // Skarsdale (Coumadin Clinic) — first name unused but provider initials matter
  ...
  // Single-name admin (Anita) — only stand-alone
  // Use a regex pass below.
];

// Word-boundary regex pass for bare last names in body text.
// Run AFTER all multi-token pairs so we don't double-replace.
const wordBoundaryPairs = [
  [/\bAnita\b/g, "Ainara"],
  // Bare last names — only the last-name token, no first name attached.
  ...
  // NOTE: Strathorne appears as patient AND in phiGuard staff allowlist (out of scope).
  // We only edit fixture files here, so converting bare "Strathorne" is safe.
  ...
  // Wood: only one occurrence (in source comment header of wood-lipid.js).
