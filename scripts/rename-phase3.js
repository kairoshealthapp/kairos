// Phase 3 systematic rename inside lib/tourScript.js
// Same mapping as Phase 1; mock-only patients (Crestwood/Marlowe/Brindelhart) excluded — they're not in the tour.
// Audio paths and slug-style internal IDs stay (Phase 6 handles audio).

const fs = require("fs");
const path = require("path");

const target = path.resolve(__dirname, "..", "lib", "tourScript.js");

// Lines matching ANY of these regexes are skipped entirely (no replacement applied to them).
// This protects audioKey, fixtureId, targetCard, data-encounter-id values which use the
// slug pattern (e.g. "aldington-tte") and must remain stable until Phase 6.
const skipLineRegexes = [
  /\baudioKey\s*:/,
  /\bfixtureId\s*:/,
  /\btargetCard\s*:/,
  /data-encounter-id=/,
...
const pairs = [
  ...
  // Beckweldon → Voronova (cardiology NP). Many forms.
...
// Word-boundary regex pass for bare last names.
...
const newLines = lines.map((line) => {
  if (skipLineRegexes.some((re) => re.test(line))) {
    skippedLines++;
    return line;
  }
  const { result, count } = applyToLine(line);
