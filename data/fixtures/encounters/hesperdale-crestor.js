// Pattern 4 — SYNTHESIS + DOSE CHANGE + LAB CLUSTER
// Source: docs/KAIROS-SESSION-2026-04-29.md Section 5 CASE 3 (MacMillan/Hesperdale)

const fixture = {
  id: "hesperdale-crestor",
  slug: "hesperdale-crestor",
  patternId: 4,
  patternName: "SYNTHESIS + DOSE CHANGE + LAB CLUSTER",
  tab: "notify",
  urgency: "calm",
  sourceChannel: "epic-result",
  sourceBox: "results-followup",
  mychartStatus: "active",
  receivedAt: "2026-04-29T12:18:00Z",
  card: {
    subject: "Lipid panel + Chem-8 — increase Crestor 20→40, repeat labs in 90d",
    kicker: "RESULTS F/U · NOTIFY",
    severity: "green",
  },
  patient: {
    name: "Hesperdale, Lorraine",
    displayName: "Lorraine Hesperdale",
    age: 55,
    sex: "F",
    dob: "1971-05-12",
    mrn: "44829137",
    proxyName: null,
    primary: "Beckweldon NP, Heart and Vascular Clinic",
    coverage: "BCBS PPO",
  },
  sourceArtifact: {
    type: "Result Note",
    author: "Beckweldon, Cardiology",
    timestamp: "2026-04-29 07:18",
    body: "Lipid panel and Chem-8 reviewed. LDL 142 (goal <100), HDL 38, TG 198, total 218. Chem-8 unremarkable. Increase Crestor (rosuvastatin) to 40 mg daily from 20 mg daily. Recheck lipid panel and hepatic function panel together in 90 days. Continue all other meds.",
  },
  initialPaneContent: {
    nurseNote: "",
    mychartMessage: "",
    phoneScript: "",
    orderPad: { orders: [], hasUnansweredQuestions: false, note: "" },
  },
  actionScripts: {
    "generate-note-mychart": [
      { type: "state-transition", target: "card", newState: "drafting", delayMsBefore: 100 },
      { type: "banner", kind: "green", text: "Pulling chart context + active medications…", durationMs: 800 },
      {
        type: "pane-update",
        target: "nurse-note",
        mode: "replace",
        typingSpeedCps: 80,
        delayMsBefore: 600,
        content:
          "Dr. Beckweldon reviewed lipid panel and Chem-8 today. LDL 142 (goal <100 in this patient with hyperlipidemia), HDL 38, triglycerides 198, total cholesterol 218. Chem-8 unremarkable. Plan: increase Crestor (rosuvastatin) to 40mg daily from 20mg daily. Repeat lipid panel and hepatic function panel together in 90 days (7/29/2026). Continue all other meds.\n\nPatient notified via MyChart.",
      },
      {
        type: "pane-update",
        target: "mychart",
        mode: "replace",
        typingSpeedCps: 70,
        delayMsBefore: 400,
        content:
          "Ms Hesperdale,\n\nDr. Beckweldon has reviewed your recent labs. Your LDL (bad cholesterol) is 142, above goal for you, while your other basic labs are normal.\n\nWe are increasing your Crestor (rosuvastatin) from 20 mg to 40 mg once daily. Continue all other medications.\n\nWe will recheck your lipid panel and a hepatic (liver) function panel together in about 90 days to make sure the higher dose is working and well tolerated. The lab orders are placed for you.\n\nPlease let us know if you have new muscle aches or dark urine.\n\nBrandon Sterne, RN BSN / Heart and Vascular Clinic",
      },
      { type: "banner", kind: "yellow", text: "Lab cluster detected: lipid + hepatic in 90 days → grouping to single date", durationMs: 1200, delayMsBefore: 200 },
      {
        type: "pane-update",
        target: "order-pad",
        mode: "instant",
        delayMsBefore: 300,
        content: {
          orders: [
            {
              type: "Discontinue Crestor 20mg daily",
              reason: "Dose adjustment",
              associatedDx: ["Other hyperlipidemia"],
              priority: "Routine",
              class: "Medication change",
              status: "Active",
              expectedDate: "2026-04-29",
              clinicalQuestions: [],
              cosign: "Beckweldon",
              auditTrail: "increasing to 40mg daily from 20mg daily per Beckweldon",
            },
            {
              type: "Crestor (rosuvastatin) 40mg daily (PO)",
              reason: "LDL 142, above goal; uptitrate",
              associatedDx: ["Other hyperlipidemia"],
              priority: "Routine",
              class: "Medication change",
              status: "Active",
              expectedDate: "2026-04-29",
              expires: "2027-04-29",
              clinicalQuestions: [],
              cosign: "Beckweldon",
            },
            {
              type: "Lipid Panel",
              codeVariant: "LIPID STERNE BR",
              reason: "follow-up after Crestor uptitration",
              associatedDx: ["Other hyperlipidemia"],
              priority: "Routine",
              class: "Lab Collect",
              status: "Future",
              expectedDate: "2026-07-29 Approx",
              clinicalQuestions: [{ q: "Fasting?", answered: true, answer: "Yes" }],
              cosign: "Beckweldon",
              labCluster: "lipid-hepatic-90d",
            },
            {
              type: "Hepatic Function Panel",
              codeVariant: "HEP STERNE BR",
              reason: "monitor LFTs on rosuvastatin 40mg",
              associatedDx: ["Other hyperlipidemia"],
              priority: "Routine",
              class: "Lab Collect",
              status: "Future",
              expectedDate: "2026-07-29 Approx",
              clinicalQuestions: [],
              cosign: "Beckweldon",
              labCluster: "lipid-hepatic-90d",
            },
          ],
          hasUnansweredQuestions: false,
        },
      },
      { type: "state-transition", target: "card", newState: "drafted", delayMsBefore: 200 },
    ],
  },
  finalSignedState: {
    nurseNote: "[As drafted above]",
    mychartMessage: "[As drafted above]",
    orders: [
      "DC Crestor 20mg",
      "Crestor 40mg daily — Active",
      "Lipid Panel — Future 7/29 (lab cluster)",
      "Hepatic Function Panel — Future 7/29 (lab cluster)",
    ],
    dxAssociated: ["Other hyperlipidemia"],
  },
};

export default fixture;
