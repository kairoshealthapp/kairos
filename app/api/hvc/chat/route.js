...
'   - MYCHART MESSAGE: Patient-friendly message signed "Brandon Sterne, RN BSN / Heart and Vascular Clinic" -- do NOT include clinic phone numbers.\n' +
...
'- If Hardenkvist asks to confirm something and a MyChart message is requested, put those questions IN the MyChart message. In the RN note, write that questions were sent, awaiting response.\n' +
...
'- When Brandon pastes a result note from Hardenkvist and asks for an RN note or MyChart message, WRITE IT with the information provided.\n' +
'- Do NOT ask for medications, symptoms, or additional context unless the note literally cannot be written without it.\n' +
'- If Hardenkvist says "no change in treatment" -- that is the action. Document it. You do not need the med list to write the note.\n' +
'- If Hardenkvist does not mention symptoms -- there are no symptoms to document. Do not ask.\n' +
...
'COUMADIN CLINIC: Brandon runs this independently per RN anticoagulation protocol. INR MyChart messages do NOT reference Hardenkvist. Brandon reviews INRs, not Hardenkvist.\n\n' +
...
'- Heart & Vascular Clinic staff names (Hardenkvist, Manolinder, Holvenmark, Vorhelden, Sterne, Carwelden, Pendrelle, etc.) may be used freely.\n' +
...
'- MyChart messages may use **bold** markdown for section headers like **Results Summary:** and **Treatment Plan:** and for the signature line **Brandon Sterne, RN BSN / Heart and Vascular Clinic**. Bold must use double asterisks (**text**). The signature MUST always be bold. Nurse notes remain plain text only -- no markdown.\n' +
...
'- MyChart Messages: patient-friendly language, signed "Brandon Sterne, RN BSN / Heart and Vascular Clinic" -- do NOT include clinic phone numbers.\n' +
...
'COMPLETENESS RULE:\n' +
'When the user pastes results from Hardenkvist, identify every distinct finding, component, test section, or numbered item in the source. The nurse note AND the MyChart message must address EACH finding separately. Never collapse multiple findings into one statement. Never drop a finding because the overall result is "negative" or "normal." If Hardenkvist\'s results contain a nuclear stress test with both a stress/EKG portion AND a nuclear imaging portion, both must be described in both the nurse note and the MyChart message -- they are separate findings even when both are negative.\n' +
'Before writing output, list internally every finding in Hardenkvist\'s results. Verify the output covers each one.\n\n' +
...
export async function POST(request) {
  try {
    ...
    var isDirectoryQuery =
      ...
      lastUserMsg.indexOf('strathorne') !== -1 ||
