...
const INTERNAL_MEDICINE_TOUR = {
  ...
  narration:
    ...
    "Take this osteoporosis screening gap. It traces to the Bone Health and Osteoporosis Foundation 2022 update, with USPSTF clinical-judgment guidance for men in this age band — DEXA bone density scan recommended for men age seventy and older without one in the last five years. The rule fired because the chart shows a seventy-one-year-old man with no DEXA on file. The provider can read the rule, see why it fired, and trace the citation back to the source. And if they disagree with the rule's interpretation, they override. Every chart commit is human-authorized. " +
...
const PULMONOLOGY_TOUR = {
  ...
  narration:
    ...
    "Take this controlling blood pressure gap. It traces to the 2017 ACC AHA hypertension guideline and the HEDIS CBP measure specification — adults with hypertension should achieve a BP below one-forty over ninety. The rule fired because the chart shows essential hypertension with the most recent BP at one-forty-six over eighty-eight, above target. And this is the cross-clinic firing pattern in action — pulmonology column, hypertension gap surfaced, a deterministic rule from the family-practice HEDIS stack firing on the right patient regardless of specialty column. Every chart commit is human-authorized. " +
