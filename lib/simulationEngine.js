...
export async function* runScript(script) {
  ...
  for (const event of script) {
    if (event && typeof event.delayMsBefore === "number" && event.delayMsBefore > 0) {
      // Pass D Phase 7 — pane-update queue delays cut by 2/3 so the
      // gap between artifact reveals tightens to feel responsive
      // alongside the 4× typing speed-up. State-transition and banner
      // events keep their authored delays because those carry timed
      // visual pacing (banner dwell, card-state transitions) rather
      // than type-engine queue startup.
      let delay = event.delayMsBefore;
      if (event.type === "pane-update") {
        delay = Math.round(delay / 3);
      }
      if (delay > 0) await skipAwareSleep(delay);
    }
    yield event;
  }
}
