...
// Pass G-fix #1 — Camera-bounce suppression. Returns true when the
// target element is "comfortably visible" in the current viewport: its
// vertical center is on-screen and at least `requiredFraction` of its
// height is on-screen. Used by cameraGoto's `skipIfVisible` option and
// by SpotlightOverlay so consecutive spotlights on already-visible
// content don't trigger an unnecessary scroll. The fraction default
// (0.6) lets a partially-clipped pane skip a re-center when most of it
// is already on-screen — the previous behavior of a hard "fully in
// viewport" check forced a re-center any time even one pixel was cut
// off, producing the seasick bounce.
export function isElementInViewport(target, options = {}) {
  if (typeof window === "undefined") return false;
  const el = resolveTarget(target);
  if (!el) return false;
  const { requiredFraction = 0.6, marginTop = 0, marginBottom = 0 } = options;
  const rect = el.getBoundingClientRect();
  if (rect.height <= 0 || rect.width <= 0) return false;
  const vh = window.innerHeight;
  const visibleTop = Math.max(rect.top, marginTop);
  const visibleBottom = Math.min(rect.bottom, vh - marginBottom);
  const visibleHeight = Math.max(0, visibleBottom - visibleTop);
  const fraction = visibleHeight / rect.height;
  const centerY = rect.top + rect.height / 2;
  const centerVisible = centerY >= marginTop && centerY <= vh - marginBottom;
  return fraction >= requiredFraction && centerVisible;
}
...
export async function cameraGoto(target, options = {}) {
  ...
  if (skipIfVisible && isElementInViewport(el, { requiredFraction: visibilityFraction })) {
    if (holdMs > 0) await new Promise((r) => setTimeout(r, holdMs));
    return;
  }
  if (framing === "tight") {
